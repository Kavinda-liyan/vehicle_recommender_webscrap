...
def parse_vehicle_title(title):
    ...
    return {"Manufacturer": manufacturer, "Model": model, "Year": year, "Condition": condition,"Fuel Type":"Hybrid"}


#Web Scraper
base_url = "https://ikman.lk/en/ads/sri-lanka/cars?enum.fuel_type=hybrid"
page = 1
hybrid_vehicles = []
...
print(f"Total hybrid vehicles found: {len(hybrid_vehicles)}")

#CSV
df = pd.DataFrame(hybrid_vehicles)
...
df.to_csv("./datasets/hybrid_vehicles_c.csv", index=False)
print(f"Cleaned data saved tohybrid_vehicles.csv with {len(df)} unique vehicles")
